...
>>>>>>> cdcf0b65235005021dfb6412df5dde6525c1ba68
        if (apiLevel <= 22) {
...
function notifyArtMethodReplaced (method) {
...
function ensureArtKnowsHowToHandleReplacementMethods () {
...
const ART_QUICK_CODE_HOOK_REDIRECT_SIZE_X86 = 5;
...
function writeArtQuickCodeReplacementTrampolineIA32 (trampoline, target, replacementMethod, vm) {
  ...
  Memory.patchCode(trampoline, 128, code => {
     ...
     *  --- Stack check ---
     *
     * If the caller is our own JNI stub, then a transition must have been pushed
     * onto the current thread's linked list.
     *
     * We invoke the original stub if the following conditions are met:
     *   1- The current managed stack is empty.
     *   2- The ArtMethod* of the linked managed stack's top frame is the same as
     *      our replacement.
     */
    ...
    // Set flag to invoke the replacement.
    writer.putMovRegU32('ecx', 1);

    // eax = self->managed_stack.tagged_top_quick_frame_.tagged_sp_
    writer.putMovRegFsU32Ptr('eax', threadOffsets.managedStack);
    writer.putTestRegReg('eax', 'eax');
    writer.putJccShortLabel('jne', 'stack_check_finished', 'no-hint');

    // eax = self->managed_stack.link_
    writer.putMovRegFsU32Ptr('eax', threadOffsets.managedStack + pointerSize);
    writer.putTestRegReg('eax', 'eax');
    writer.putJccShortLabel('je', 'stack_check_finished', 'unlikely');

    // eax = eax->tagged_top_quick_frame_.tagged_sp_
    writer.putMovRegRegPtr('eax', 'eax');
    writer.putTestRegReg('eax', 'eax');
    writer.putJccShortLabel('je', 'stack_check_finished', 'no-hint');

    // eax = *eax  (ArtMethod *)
    writer.putMovRegRegOffsetPtr('eax', 'eax', -1);
    writer.putMovRegAddress('ebx', replacementMethod);
    writer.putCmpRegReg('eax', 'ebx');
    writer.putJccShortLabel('jne', 'stack_check_finished', 'no-hint');

    writer.putXorRegReg('ecx', 'ecx');

    writer.putLabel('stack_check_finished');
    writer.putTestRegReg('ecx', 'ecx');
    ...
    } while (offset < ART_QUICK_CODE_HOOK_REDIRECT_SIZE_X86);
    ...
    // Swap out the ArtMethod * with our replacement.
    writer.putMovRegAddress('eax', replacementMethod);

    // Jump to the GenericJniTrampoline to transition into our NativeCallback.
...
function writeArtQuickCodeReplacementTrampolineArm64 (trampoline, target, replacementMethod, vm) {
  const threadOffsets = getArtThreadSpec(vm).offset;
  const artMethodOffsets = getArtMethodSpec(vm).offset;
  ...
  Memory.patchCode(trampoline, 128, code => {
    ...
    writer.putPushRegReg('x0', 'x16');

    // Set flag to invoke the replacement.
    writer.putLdrRegU64('x15', 1);

    // x0 = self->managed_stack.tagged_top_quick_frame_.tagged_sp_
    writer.putAddRegRegImm('x16', 'x19', threadOffsets.managedStack);
    writer.putLdrRegRegOffset('x0', 'x16', 0);
    writer.putCbnzRegLabel('x0', 'stack_check_finished');

    // x0 = self->managed_stack.link_
    writer.putLdrRegRegOffset('x0', 'x16', pointerSize);
    writer.putCbzRegLabel('x0', 'stack_check_finished');

    // x0 = x0->tagged_top_quick_frame_.tagged_sp_
    writer.putLdrRegRegOffset('x0', 'x0', 0);
    writer.putCbzRegLabel('x0', 'stack_check_finished');

    // x0 = *x0  (ArtMethod *)
    writer.putSubRegRegImm('x0', 'x0', 1);
    writer.putLdrRegRegOffset('x0', 'x0', 0);
    writer.putLdrRegAddress('x16', replacementMethod);
    writer.putSubRegRegReg('x0', 'x0', 'x16');
    writer.putCbnzRegLabel('x0', 'stack_check_finished');

    writer.putMovRegReg('x15', 'xzr');

    writer.putLabel('stack_check_finished');

    writer.putPopRegReg('x0', 'x16');

    writer.putCbnzRegLabel('x15', 'invoke_replacement');
    ...
    if (!relocator.eoi) {
      writer.putLdrRegAddress('x16', target.add(offset));
      writer.putBrReg('x16');
    }
    ...
    // Swap out the ArtMethod * with our replacement.
    writer.putLdrRegAddress('x0', replacementMethod);
...
class ArtMethodMangler {
  ...
  replace (impl, isInstanceMethod, argTypes, vm, api) {
    ...
    const prologueLength = writeTrampoline(this.trampoline, quickCode, replacementMethod, vm);
    this.overwrittenPrologueLength = prologueLength;

    this.overwrittenPrologue = Memory.dup(quickCode, prologueLength);

    const writePrologue = artQuickCodePrologueWriters[Process.arch];
    writePrologue(quickCode, this.trampoline);

    notifyArtMethodReplaced(replacementMethod);

    /* ----  Debugging logic  ---- */
    /*Interceptor.attach(this.originalMethod.quickCode, function(args) {
      console.log(`QuickCode called.`);
      const backtrace = Thread.backtrace(this.context).map(DebugSymbol.fromAddress).join('\n\t\t');
      console.log('\tCalled from:\n\t\t' + backtrace);
      console.log(`sp: ${this.context.sp}. return_addr: ${this.context.sp.readPointer()}`);
    });
    console.log(`replacementMethod: ${replacementMethod} hookedMethod: ${hookedMethod}`);
    console.log(`entrypont: ${entrypoint}`);

    Interceptor.attach(Module.getExportByName('libart.so', '_ZN3art6Thread21QuickDeliverExceptionEv'), function (args) {
      console.log(`=== Thread::QuickDeliverException ===`);
      const backtrace = Thread.backtrace(this.context).map(DebugSymbol.fromAddress).join('\n\t\t');
      console.log('\tCalled from:\n\t\t' + backtrace);
    });

    Interceptor.attach(Module.getExportByName('libart.so', '_ZN3art9ArtMethod14FindCatchBlockENS_6HandleINS_6mirror5ClassEEEjPb'), {
      onEnter: function(args) {
        console.log(`\t>>> ArtMethod::FindCatchBlock`);
        console.log(`\t\tArtMethod: addr(${args[0]}), name(${new ArtMethod(args[0]).prettyMethod()})`)
      },
    ...
    Interceptor.attach(Module.getExportByName('libart.so', '_ZNK3art20OatQuickMethodHeader15ToNativeQuickPcEPNS_9ArtMethodEjbb'), {
      onEnter: function(args) {
        console.log("\t>>> OatQuickMethodHeader::ToNativeQuickPc");
      },
      onLeave: function(retval) {
        console.log(`\t\tretval: ${retval}`);
        console.log(`\t<<< OatQuickMethodHeader::ToNativeQuickPc`);
    ...
    Interceptor.attach(Module.getExportByName('libart.so', '_ZN3art21QuickExceptionHandler9FindCatchENS_6ObjPtrINS_6mirror9ThrowableEEE'), {
      onEnter: function(args) {
        console.log(`\t>>> QuickExceptionHandler::FindCatch`);
      },
      onLeave: function(retval) {
        console.log(`\t<<< QuickExceptionHandler::FindCatch`);
      }
    ...
    const getMethod = new NativeFunction(DebugSymbol.fromName('_ZNK3art12StackVisitor9GetMethodEv').address,
        'pointer', ['pointer'], nativeFunctionOptions);

    Interceptor.attach(DebugSymbol.fromName('_ZN3art22CatchBlockStackVisitor10VisitFrameEv').address, {
      onEnter: function(args) {
        console.log('\t>>> CatchBlockStackVisitor::VisitFrame');
        const stackVisitor = args[0];
        const curQuickFrame = stackVisitor.add(4 * pointerSize).readPointer();
        const curQuickFramePc = stackVisitor.add(5 * pointerSize).readPointer();
        const curOatQuickMethodHdr = stackVisitor.add(6 * pointerSize).readPointer();
        const method = getMethod(stackVisitor);
        let methodName = "";
        if (!method.isNull())
          methodName = new ArtMethod(method).prettyMethod();

        console.log(`\t\tcur_quick_frame_: ${curQuickFrame}`);
        console.log(`\t\tcur_quick_frame_pc_: ${curQuickFramePc}`);
        console.log(`\t\tcur_oat_quick_method_header_: ${curOatQuickMethodHdr}`);
        console.log(`\t\tArtMethod: ${method}, name(${methodName})`);
        if (method.equals(replacementMethod)) {
          let i = 0;
          let sp = curQuickFrame;

          console.log('Dumping stack frame');
          console.log(`\t\t\t#${sp}: ${DebugSymbol.fromAddress(sp.readPointer()).toString()}`);
          i += 4

          for (let j = 0; j < 4; j++) {
            console.log(`\t\t\t#${sp.add(i)}: ${sp.add(i).readU64()}`);
            i += 8;
          }
          for (let j = 0; j < 7; j++) {
            console.log(`\t\t\t#${sp.add(i)}: ${DebugSymbol.fromAddress(sp.add(i).readPointer()).toString()}`);
            i += 4;
          }
        }
      },
  ...
  revert (vm) {
    Memory.copy(this.originalMethod.quickCode, this.overwrittenPrologue, this.overwrittenPrologueLength);
  }
